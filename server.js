...
function extractAvatarFromChannelData(data) {
...
async function lookupKickAvatar(username) {
  ...
  const urls = [
    `https://kick.com/api/v2/channels/${encodeURIComponent(name)}`,
    `https://kick.com/api/v1/channels/${encodeURIComponent(name)}`
  ];
  ...
  for (const url of urls) {
    try {
      ...
      const avatar = extractAvatarFromChannelData(data);
...
async function fillAvatarForUser(username) {
  const key = String(username || '').toLowerCase();
  if (!key || !state.entries.has(key)) return;

  const entry = state.entries.get(key);
  if (entry.avatar) return;

  const avatar = await lookupKickAvatar(entry.username);
  if (!avatar) return;

  entry.avatar = avatar;

  if (state.lastWinner && state.lastWinner.toLowerCase() === key) {
    state.lastWinnerAvatar = avatar;
  }

  io.emit('avatar-updated', { username: entry.username, avatar, state: publicState() });
  broadcast();
}
...
function publicState() {
  ...
  return {
    ...
    winnerMessages: state.winnerMessages.slice(0, 20),
...
function addEntry(username, avatar = null) {
  ...
  if (state.entries.has(key)) {
    ...
    if (!existing.avatar && avatar) {
      ...
      io.emit('avatar-updated', { username: existing.username, avatar, state: publicState() });
      broadcast();
    } else if (!existing.avatar) {
      fillAvatarForUser(existing.username).catch(() => {});
    }
    return;
  }
  ...
  if (!entry.avatar) {
    fillAvatarForUser(display).catch(() => {});
  }
}
...
function handleChatMessage(payload) {
  ...
  if (
    state.lastWinner &&
    !state.rolling &&
    String(username).toLowerCase() === String(state.lastWinner).toLowerCase()
  ) {
    const msg = { username, message, at: new Date().toISOString() };
    state.winnerMessages.unshift(msg);
    state.winnerMessages = state.winnerMessages.slice(0, 20);
    io.emit('winner-message', { message: msg, state: publicState() });
    broadcast();
  }
...
app.post('/api/roll', requirePin, async (req, res) => {
  let details = participantDetails();

  await Promise.all(details.slice(0, 100).map(async d => {
    if (!d.avatar) {
      const avatar = await lookupKickAvatar(d.username).catch(() => null);
      if (avatar) {
        const stored = state.entries.get(String(d.username).toLowerCase());
        if (stored) stored.avatar = avatar;
        d.avatar = avatar;
      }
    }
  }));
  ...
  let winnerAvatar = winnerDetail?.avatar || null;
  if (!winnerAvatar) {
    winnerAvatar = await lookupKickAvatar(winner).catch(() => null);
    const stored = state.entries.get(String(winner).toLowerCase());
    if (stored && winnerAvatar) stored.avatar = winnerAvatar;
    if (winnerDetail && winnerAvatar) winnerDetail.avatar = winnerAvatar;
  }
