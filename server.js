...
function publicState() {
  const participants = [...state.entries.values()].map(x => x.username).sort((a, b) => a.localeCompare(b));
  return {
    ...
    participants,
    participantDetails: [...state.entries.values()].map(x => ({ username: x.username, avatar: x.avatar || null })),
...
function handleChatMessage(payload) {
  ...
  // Keep the streamer's Kick avatar for the default giveaway screen.
  if (String(username).toLowerCase() === String(KICK_CHANNEL).toLowerCase() && avatar) {
    if (state.channelAvatar !== avatar) {
...
app.post('/api/roll', requirePin, (req, res) => {
  const participants = [...state.entries.values()].map(x => x.username);
  if (!participants.length) return res.status(400).json({ ok: false, error: 'No participants' });

  const winner = participants[Math.floor(Math.random() * participants.length)];
  const winnerEntry = state.entries.get(String(winner).toLowerCase());
  const winnerAvatar = winnerEntry?.avatar || null;
