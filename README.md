...
## Default streamer profile picture

The default giveaway-ready screen now uses the Kick channel owner's profile picture instead of the `W` placeholder. The app captures the profile picture from the same live Kick chat payload when the channel owner chats. For `KICK_CHANNEL=w`, once `w` sends a chat message, the default screen updates to that Kick profile picture automatically.
